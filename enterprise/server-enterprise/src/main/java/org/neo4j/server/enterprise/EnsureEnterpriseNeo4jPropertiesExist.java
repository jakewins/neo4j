...
public class EnsureEnterpriseNeo4jPropertiesExist extends EnsureNeo4jPropertiesExist
{
    ...
    {
        String dbMode = config.get( ServerSettings.db_mode );
        ...
        {
            failureMessage = String.format( "Illegal value for %s \"%s\" in %s", ServerSettings.db_mode.name(), dbMode,
                    ServerInternalSettings.SERVER_CONFIG_FILE_KEY );
            return false;
        }

        final File dbTuningFile = config.get( ServerSettings.db_config );
