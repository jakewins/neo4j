...
import static org.neo4j.kernel.impl.enterprise.lock.f2.F2Lock.AcquireOutcome.ACQUIRED;
import static org.neo4j.kernel.impl.enterprise.lock.f2.F2Lock.AcquireOutcome.MUST_WAIT;
import static org.neo4j.kernel.impl.enterprise.lock.f2.F2Lock.AcquireOutcome.NOT_ACQUIRED;
import static org.neo4j.kernel.impl.enterprise.lock.f2.F2Lock.ReleaseOutcome.LOCK_IDLE;
import static org.neo4j.kernel.impl.enterprise.lock.f2.F2Lock.ReleaseOutcome.LOCK_HELD;
...
class F2Lock
{
    ...
    F2ClientEntry exclusiveHolder;

    /** Linked list of holders of shared lock */
    F2ClientEntry sharedHolderList;

    /** Linked list of waiting holders */
    F2ClientEntry waitList;
    ...
    {
        assert entry.owner.waitsFor == null :
                String.format( "Client marked as waiting, cannot acquire: Client(%s).waitsFor = %s, acquiring=%s", entry.owner, entry.owner.waitsFor, entry );
    ...
    {
        return "Lock(" + resourceType.name() + "[" + resourceId + "])";
    }
