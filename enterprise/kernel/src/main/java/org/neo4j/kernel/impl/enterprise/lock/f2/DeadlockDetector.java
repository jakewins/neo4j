...
class DeadlockDescription
{
    ...
    public String toString()
    {
        ...
        return sb.append( "\n" ).append( "Where:\n" ).append( aliases.describe() ).toString();
    }
    ...
    private static boolean assertIsValidDeadlockChain( F2ClientEntry[] chain )
    {
        ...
        for ( int linkIndex = 1; linkIndex < chain.length; linkIndex++ )
        {
            ...
            assert waiter.owner != holder.owner :
                    String.format( "Invalid deadlock chain: %s\n" + "%s and %s are same client.", Arrays.toString( chain ), waiter, holder );

            // The waiter should be the same client as the holder from the previous iteration
            assert lastHolder == null || waiter.owner == lastHolder.owner :
                    String.format( "Invalid deadlock chain: %s\n" + "LastHolder(%s) and Waiter(%s) are not the same client.", Arrays.toString( chain ),
                            lastHolder, waiter );
    ...
    private static void assertLockHeldBy( F2Lock lock, F2Client expectedHolder )
    {
        if ( lock.exclusiveHolder.owner != expectedHolder )
        {
            ...
            assert isHoldingSharedLock :
                    String.format( "Expected %s to hold shared or exclusive lock on %s. " + "Exclusive holder is %s, shared holder HEAD is %s", expectedHolder,
                            lock, lock.exclusiveHolder, lock.sharedHolderList );
        }
    }

    private static class ClientAliaser
    {
        private static final String[] aliases = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split( "" );
        ...
        if ( lock.exclusiveHolder != null )
        {
            ...
            if ( lock.exclusiveHolder.owner == blockee )
            {
                throw new RuntimeException(
                        String.format( "Uhhh %s, waitsFor:%s source:%s", lock.exclusiveHolder, lock.exclusiveHolder.owner.waitsFor, source ) );
            }
