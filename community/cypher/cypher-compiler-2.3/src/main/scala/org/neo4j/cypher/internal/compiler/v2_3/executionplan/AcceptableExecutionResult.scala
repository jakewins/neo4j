 ...
 */
abstract class AcceptableExecutionResult(taskCloser: TaskCloser, statement: Statement)
  ...
  override final def javaColumnAs[T](column: String): ResourceIterator[T] = new WrappingResourceIterator[T] {
    def hasNext = AcceptableExecutionResult.this.hasNext
  ...
  protected def populateResults(results: util.List[util.Map[String, Any]])(row: ResultRow) = {
    val map = new util.HashMap[String, Any]()
    columns.foreach(c => map.put(c, row.get(c)))
    results.add(map)
  }

  protected def populateDumpToStringResults(builder: mutable.Builder[Map[String, String], Seq[Map[String, String]]])(row: ResultRow) = {
  ...
  protected def doInAccept[T](body: ResultRow => T) = {
  ...
  protected def materializeAsScala(v: Any): Any = {
  ...
  protected trait WrappingResourceIterator[T] extends ResourceIterator[T] {
