...
public abstract class StateHandlingStatementOperations implements StatementLayer
{
    private final LegacyPropertyTrackers legacyPropertyTrackers;
    private final ConstraintIndexCreator constraintIndexCreator;

    public StateHandlingStatementOperations( LegacyPropertyTrackers propertyTrackers,
    ...
    public void relationshipDelete( KernelStatement state, long relationshipId )
    {
        ...
        else
        {
            try
            {
                delegate().accept( relationshipId, new StatementLayer.RelationshipVisitor()
                {
                    @Override
    ...
    public boolean nodeHasLabel( KernelStatement state, long nodeId, int labelId ) throws EntityNotFoundException
    {
        ...
        return delegate().nodeHasLabel( state, nodeId, labelId );
    }
    ...
    public PrimitiveIntIterator nodeGetLabels( KernelStatement state, long nodeId ) throws EntityNotFoundException
    {
        if ( state.hasTxStateWithChanges() )
        {
            ...
            return state.txState().nodeStateLabelDiffSets( nodeId ).applyPrimitiveIntIterator(
                    delegate().nodeGetLabels( state, nodeId ) );
        }

        return delegate().nodeGetLabels( state, nodeId );
    }
        ...
        if ( state.hasTxStateWithChanges() )
        {
            PrimitiveLongIterator wLabelChanges =
                    state.txState().nodesWithLabelChanged( labelId ).applyPrimitiveLongIterator(
                            delegate().nodesGetForLabel( state, labelId ) );
            return state.txState().nodesDeletedInTx().applyPrimitiveLongIterator( wLabelChanges );
        }

        return delegate().nodesGetForLabel( state, labelId );
    }
            ...
            throws CreateConstraintFailureException
    {
        ...
        try
        {
            ...
            { // create from scratch
                for ( Iterator<UniquenessConstraint> it = delegate().constraintsGetForLabelAndPropertyKey(
    ...
    {
        return applyConstraintsDiff( state, delegate().constraintsGetForLabelAndPropertyKey(
    ...
    {
        return applyConstraintsDiff( state, delegate().constraintsGetForLabel( state, labelId ), labelId );
    }

    @Override
    public Iterator<UniquenessConstraint> constraintsGetAll( KernelStatement state )
    {
        return applyConstraintsDiff( state, delegate().constraintsGetAll( state ) );
    }
            ...
            throws SchemaRuleNotFoundException
    {
        ...
        try
        {
            committedRules = option( delegate().indexesGetForLabelAndPropertyKey( state, labelId,
                    propertyKey ) );
        }
    ...
    public Iterator<IndexDescriptor> indexesGetForLabel( KernelStatement state, int labelId )
    {
        if ( state.hasTxStateWithChanges() )
        {
            return state.txState().indexDiffSetsByLabel( labelId )
                    .apply( delegate().indexesGetForLabel( state, labelId ) );
        }
    ...
    public Iterator<IndexDescriptor> indexesGetAll( KernelStatement state )
    {
        if ( state.hasTxStateWithChanges() )
        {
            return state.txState().indexChanges().apply( delegate().indexesGetAll( state ) );
        }
    ...
    public Iterator<IndexDescriptor> uniqueIndexesGetForLabel( KernelStatement state, int labelId )
    {
        if ( state.hasTxStateWithChanges() )
        {
            return state.txState().constraintIndexDiffSetsByLabel( labelId )
                    .apply( delegate().uniqueIndexesGetForLabel( state, labelId ) );
        }
    ...
    public Iterator<IndexDescriptor> uniqueIndexesGetAll( KernelStatement state )
    {
        if ( state.hasTxStateWithChanges() )
        {
            return state.txState().constraintIndexChanges()
                    .apply( delegate().uniqueIndexesGetAll( state ) );
        }
    ...
    {
//        PrimitiveLongIterator committed = delegate().nodeGetUniqueFromIndexLookup( state, index, value );
...
//        PrimitiveLongIterator changeFilteredMatches = filterIndexStateChanges( state, index, value, exactMatches );
    ...
    public PrimitiveLongIterator nodesGetFromIndexLookup( KernelStatement state, IndexDescriptor index, final Object value )
            throws IndexNotFoundKernelException
    {
        PrimitiveLongIterator committed = delegate().nodesGetFromIndexLookup( state, index, value );
        PrimitiveLongIterator exactMatches = filterExactIndexMatches( state, index, value, committed );
        PrimitiveLongIterator changeFilteredMatches = filterIndexStateChanges( state, index, value, exactMatches );
    ...
    public PrimitiveLongIterator nodeGetPropertyKeys( KernelStatement state, long nodeId ) throws EntityNotFoundException
    {
        ...
        return delegate().nodeGetPropertyKeys( state, nodeId );
    }
            ...
            throws EntityNotFoundException
    {
        ...
        return delegate().nodeGetProperty( state, nodeId, propertyKeyId );
    }
            ...
            throws EntityNotFoundException
    {
        if ( state.hasTxStateWithChanges() )
        {
            ...
            return state.txState().nodePropertyDiffSets( nodeId )
                    .apply( delegate().nodeGetAllProperties( state, nodeId ) );
        }

        return delegate().nodeGetAllProperties( state, nodeId );
    }
            ...
            throws EntityNotFoundException
    {
        ...
        return delegate().relationshipGetPropertyKeys( state, relationshipId );
    }
            ...
            throws EntityNotFoundException
    {
        ...
        return delegate().relationshipGetProperty( state, relationshipId, propertyKeyId );
    }
            ...
            throws EntityNotFoundException
    {
        if ( state.hasTxStateWithChanges() )
        {
            ...
            return state.txState().relationshipPropertyDiffSets( relationshipId )
                    .apply( delegate().relationshipGetAllProperties( state, relationshipId ) );
        }
        else
        {
            return delegate().relationshipGetAllProperties( state, relationshipId );
        }
    }
    ...
    public PrimitiveLongIterator graphGetPropertyKeys( KernelStatement state )
    {
        ...
        return delegate().graphGetPropertyKeys( state );
    }
    ...
    public Iterator<DefinedProperty> graphGetAllProperties( KernelStatement state )
    {
        if ( state.hasTxStateWithChanges() )
        {
            return state.txState().graphPropertyDiffSets().apply( delegate().graphGetAllProperties( state ) );
        }

        return delegate().graphGetAllProperties( state );
    }
                                                       ...
                                                       int[] relTypes ) throws EntityNotFoundException
    {
        ...
        if ( state.hasTxStateWithChanges() )
        {
            ...
            {
                stored = delegate().nodeGetRelationships( state, nodeId, direction, relTypes );
            }
            return txState.augmentRelationships( nodeId, direction, relTypes, stored );
        }
        return delegate().nodeGetRelationships( state, nodeId, direction, relTypes );
    }
    ...
    public PrimitiveLongIterator nodeGetRelationships( KernelStatement state, long nodeId, Direction direction ) throws EntityNotFoundException
    {
        if ( state.hasTxStateWithChanges() )
        {
            ...
            {
                stored = delegate().nodeGetRelationships( state, nodeId, direction );
            }
            return txState.augmentRelationships( nodeId, direction, stored );
        }
        return delegate().nodeGetRelationships( state, nodeId, direction );
    }
...

    {
        if( state.hasTxStateWithChanges() )
        {
            ...
            if( !state.txState().nodeIsAddedInThisTx( nodeId ))
            {
                degree = delegate().nodeGetDegree( state, nodeId, direction, relType );
            }
        ...
        {
            return delegate().nodeGetDegree( state, nodeId, direction, relType );
        }
    }
    ...
    public int nodeGetDegree( KernelStatement state, long nodeId, Direction direction ) throws EntityNotFoundException
    {
        if( state.hasTxStateWithChanges() )
        {
            ...
            if( !state.txState().nodeIsAddedInThisTx( nodeId ))
            {
                degree = delegate().nodeGetDegree( state, nodeId, direction );
            }
        ...
        {
            return delegate().nodeGetDegree(state,  nodeId, direction );
        }
    }
            ...
            throws EntityNotFoundException
    {
        if(statement.hasTxStateWithChanges() && statement.txState().nodeModifiedInThisTx(nodeId))
        {
            ...
            // in current tx.
            PrimitiveIntIterator committedTypes = delegate().nodeGetRelationshipTypes( statement, nodeId );
        ...
        {
            return delegate().nodeGetRelationshipTypes( statement, nodeId );
        }
    }
