...
(function() {
    ...
    Require.paths = function() {
        var r = [];
        r.push( Require.root + "/node_modules/babel-core/node_modules" );
    ...
    function findRoot(parent) {
        ...
        pathParts.forEach( function(part){
            if( part === ".." ) { canonical.pop(); }
            else if( part === ".") { /* No-op */ }
            else { canonical.push( part ); }
        } )

        var root = canonical.join('/');
        print( "findRoot> " + parent.id + " -> " + root );
        return root;
    }
    ...
    function resolveAsNodeModule(id, root) {
        ...
        if(Require.debug) print( "nodemodule> " + id + " [" + root + "]");
    ...
    function resolveAsDirectory(id, root) {
        var base = [root, id].join('/'),
            file = [base, 'package.json'].join('/');

        var classloader = java.lang.Thread.currentThread().getContextClassLoader();
        if (classloader.findResource(file)) {
            try {
