...
public class NeoStoreIndexStoreViewTest
{
    ...
    private void getOrCreateIds() throws SchemaKernelException
    {
        ...
        try
        {
            ...
            KernelStatement ctx = bridge.getCtxForWriting();
            labelId = ctx.keyWriteOperations().labelGetOrCreateForName( "Person" );
            propertyKeyId = ctx.keyWriteOperations().propertyKeyGetOrCreateForName( "name" );
            ctx.close();
            tx.success();
        }
        finally
