...
//        test( Byte.MIN_VALUE, IndexOutput::writeByte, IndexInput::readByte );
    ...
    private <T> void testRandomAccess( T val, ThrowingBiConsumer<IndexOutput,T,IOException> write,
            ThrowingBiFunction<RandomAccessInput,Long,T,IOException> read ) throws IOException
    {
    ...
    public void shouldGiveCorrectPosition() throws Exception
    {
        ...
        try ( IndexInput in = dir.openInput( fileName, IOContext.READ ) )
        {
            ...
            long actual = in.getFilePointer();
            assert actual == fileLen : String.format( "Expected position to be %s, got %s", fileLen, actual );
        }
    }
