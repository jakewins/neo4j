...
public class NodeRangeDocumentLabelScanStorageStrategy implements LabelScanStorageStrategy
{
    ...
    public Iterator<Long> labelsForNode( IndexSearcher searcher, long nodeId )
    {
        try
        {
            ...
            for ( IndexableField fields : searcher.doc( doc ).getFields() )
            {
                ...
                if ( fields instanceof LongField )
                {
                    LongField labelField = (LongField) fields;
                    long bitmap = labelField.numericValue().longValue();
    ...
    @Override
    public LabelScanWriter acquireWriter( final StorageService storage ) throws IOException
    {
