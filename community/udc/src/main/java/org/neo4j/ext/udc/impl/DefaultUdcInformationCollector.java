...
public class DefaultUdcInformationCollector implements UdcInformationCollector
{
    ...
    private final UsageData sysInfo;
    ...
    public DefaultUdcInformationCollector( Config config, DataSourceManager xadsm,
            IdGeneratorFactory idGeneratorFactory, StartupStatistics startupStats, UsageData sysInfo )
    {
        this.config = config;
    ...
    public Map<String, String> getUdcParams()
    {
        ...
        add( udcFields, VERSION, filterVersionForUDC( sysInfo.get( UsageDataKeys.version ) ) );
        add( udcFields, REVISION, filterVersionForUDC( sysInfo.get( UsageDataKeys.revision ) ) );

        add( udcFields, EDITION, sysInfo.get( UsageDataKeys.edition ).name().toLowerCase() );
        ...
        add( udcFields, DATABASE_MODE, sysInfo.get( UsageDataKeys.operationalMode ).name() );
        add( udcFields, SERVER_ID, sysInfo.get( UsageDataKeys.serverId ) );
        add( udcFields, USER_AGENTS, toCommaString( sysInfo.get( UsageDataKeys.clientNames ) ) );
